...
const light = initDefaultSpotlight(scene, new THREE.Vector3(5.0, 5.0, 5.0)); // Use default light    
...
function keyboardUpdate() {
   ...
   listenKeyboardTranslationEvents();
   listenKeyboardRotationEvents();
   
   updateCamera();
}

function listenKeyboardTranslationEvents() {
...
function listenKeyboardRotationEvents() {
   if (keyboard.pressed('left')) {
      cameraHolder.rotateY(THREE.MathUtils.degToRad(1));
   }
   ...
   if (keyboard.pressed('up')) {
      cameraHolder.rotateX(THREE.MathUtils.degToRad(-1));
   }
   ...
   if (keyboard.pressed('Q')) {
      cameraHolder.rotateZ(THREE.MathUtils.degToRad(-1));
   }
